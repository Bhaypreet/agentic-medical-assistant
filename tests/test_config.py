...
def _settings(**overrides):
    base = {"groq_api_key": "test-key", "environment": "development", "api_keys": ""}
...
def test_production_refuses_to_start_without_api_keys():
    # Without this the service would serve patient data to anyone.
    with pytest.raises(ValueError, match="API_KEYS must be set in production"):
...
def test_production_starts_with_keys_configured():
    settings = _settings(environment="production", api_keys="one, two ,three")
